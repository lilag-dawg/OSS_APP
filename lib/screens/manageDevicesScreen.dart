...
class _ManageDevicesScreenState extends State<ManageDevicesScreen> {
  ...
  Future<void> _getListOfSensorCharact(
    ...
    await _getSensorNamesList(
            listCapteursCharact.characteristic.value, numberOfsensor)
        .then((value) async {
  ...
  Future<List<Device>> _getSensorNamesList(
      Stream<List<int>> stream, int numberOfsensor) async {
    String currentName = "";
    ...
    await for (var value in stream) {
      if (value.isNotEmpty) {
        ...
        if (listDevices
            .where((element) =>
                element.name == currentName && element.status == currentStatus)
            .isNotEmpty) {
          count = count + 1;
        } else {
          if (listDevices
              .where((element) => element.name == currentName)
              .isEmpty) {
            listDevices.add(Device(currentName, currentStatus));
          } else {
            int index = listDevices.indexOf(listDevices
                .firstWhere((element) => element.name == currentName));
  ...
  Widget _deviceIsNotOss() {
    return Text(S.current.manageDeviceScreenNotOSS);
  }

  Widget _deviceIsOss(BluetoothDeviceManager ossManager) {
    List<Device> notPairedDevices = receivedDevices
        .where((element) =>
            element.status == notPaired || element.status == connecting)
        .toList();
    List<Device> pairedDevices =
        receivedDevices.where((element) => element.status == paired).toList();
    return Container(
      child: Column(
        children: [
          _customBox(ossManager, S.current.manageDeviceScreenConnected,
              _buildDevicesTiles(notPairedDevices, ossManager)),
          _customBox(ossManager, S.current.manageDeviceScreenPaired,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _isDeviceOssCompatible(ossManager)
          ? SingleChildScrollView(child: _buildBody(ossManager))
          : _deviceIsNotOss(),
    );
  }
  ...
  List<Widget> _buildDevicesTiles(
    ...
    return result
        .map((d) => _CustomTile(
              ...
              onTapTile: (String currentStatus) {
                setState(() {
...
class _CustomTile extends StatefulWidget {
  ...
  const _CustomTile({Key key, this.ossManager, this.onTapTile, this.device})
      : super(key: key);
...
class __CustomTileState extends State<_CustomTile> {
  ...
  Future<void> _writeToMCU(
      String status, BluetoothDeviceManager ossManager) async {
    ...
    await paringRequestCharact.characteristic.write(
        BluetoothDeviceManager.sendPairingRequestCharact(
            widget.device.name, status));
  }

  Future<void> _handlePressTrailing(
      String status, BluetoothDeviceManager ossManager) async {
  ...
  Future<String> _waitForSensorConnection(Stream<List<int>> stream) async {
    ...
    await for (var value in stream) {
      ...
      if (widget.device.name == currentName) {
        ...
        if (currentStatus == paired) break;
      }
    }
    return currentStatus;
  }

  Future<SupportedDataType> _getSensorDataTypes(
      Stream<List<int>> stream, String deviceName) async {
    String currentName = "";
    SupportedDataType supportedDataType =
        SupportedDataType(false, false, false, false, false);
    await for (var value in stream) {
      currentName =
          BluetoothDeviceManager.convertRawToStringListCapteursCharact(value);
  ...
  Future<void> _getSensorDataTypeFromCharact(
      BluetoothDeviceManager ossManager) async {
    BluetoothDeviceCharacteristic sensorSupportedData = ossManager.ossDevice
        .getService(BluetoothDeviceManager.connectionHandlingService)
        .getCharacteristic(BluetoothDeviceManager.sensorDataType);
    await sensorSupportedData.characteristic.setNotifyValue(true);
    await _getSensorDataTypes(
            sensorSupportedData.characteristic.value, widget.device.name)
        .then((value) async {
  ...
  Future<void> _waitForSensorStringListFromCharact(
      BluetoothDeviceManager ossManager) async {
    BluetoothDeviceCharacteristic listCapteursCharact = ossManager.ossDevice
        .getService(BluetoothDeviceManager.connectionHandlingService)
        .getCharacteristic(BluetoothDeviceManager.listCapteursCharact);
    ...
    if (listCapteursCharact != null) {
      await listCapteursCharact.characteristic.setNotifyValue(true);
      await _waitForSensorConnection(listCapteursCharact.characteristic.value)
          .then((value) async {
        //ajouter timeout pour rendre plus robuste
        currentStatus = value;
        await listCapteursCharact.characteristic
            .setNotifyValue(false)
            .then((value) => widget.onTapTile(currentStatus));
      });
    }
  }
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: SingleChildScrollView(
              child: ListBody(
                children: <Widget>[
                  Text(S.current.manageDeviceScreenListOfFeatures),
                  CheckboxListTile(
                      title: Text(S.of(context).manageDeviceScreenPower),
                      value: device.supportedDataType.power,
                      onChanged: null),
                  CheckboxListTile(
                      title: Text(S.of(context).manageDeviceScreenCadence),
                      value: device.supportedDataType.cadence,
                      onChanged: null),
                  CheckboxListTile(
                      title: Text(S.of(context).manageDeviceScreenSpeed),
                      value: device.supportedDataType.speed,
                      onChanged: null),
                  CheckboxListTile(
                      title: Text(S.of(context).manageDeviceScreenBattery),
                      value: device.supportedDataType.battery,
                      onChanged: null),
                  CheckboxListTile(
                      title: Text(S.of(context).manageDeviceScreenGear),
        ...
        ? FutureBuilder<void>(
            ...
            builder: (c, snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                return RaisedButton(
                    child: (status == paired)
                        ? Text(S.of(context).manageDeviceScreenForget)
                        : Text(S.of(context).manageDeviceScreenPair),
                    onPressed: () async {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      subtitle: (status == paired)
          ? Text(S.of(context).manageDeviceScreenActive)
          : SizedBox.shrink(),
      trailing: _customTrailing(widget.device.name, status, widget.ossManager),
      onTap: () async {
        if (status == paired) {
          await _getSensorDataTypeFromCharact(widget.ossManager)
              .then((value) async {
...
class SupportedDataType {
  ...
  SupportedDataType(
      bool cadence, bool speed, bool battery, bool power, bool gear) {
